...
def ask_question(documents, question, chat_history):
    ...
    for doc_name, doc_data in documents.items():
        for page in doc_data["pages"]:
            total_tokens += calculate_token_count(
                page.get("text_summary", "No summary available")
            )
            total_tokens += calculate_token_count(
    ...
    def check_page_relevance(doc_name, page):
        ...
        Check if the document's page content given below has any relevant content that can be to answer the user's question.
        Here's the full text and image analysis of a page from the document:
        ...
        Given below is the user's question.
        ---
        User's question: {preprocessed_question}
        ---

        Respond with "yes" if this page contains any relevant information, otherwise respond with "no".
        """
        ...
        relevance_data = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are an assistant that determines if a given content is relevant to a question.",
                },
        ...
        for attempt in range(max_retries):
            try:
                ...
                logging.error(
                    f"Error checking relevance of page {page['page_number']} in '{doc_name}': {e}. Attempt {attempt + 1} of {max_retries}"
                )
    ...
    shrunk_combined_relevant_content = ""
    for page in relevant_pages:
        combined_relevant_content += (
            f"\nDocument name: {page['doc_name']}, Page number: {page['page_number']}\n"
            f"Summary: {page['text_summary']}\n"
            f"Full Text: {page['full_text']}\n"
            f"Image Analysis: {page['image_explanation']}\n"
        )
        shrunk_combined_relevant_content += (
            f"\nDocument: {page['doc_name']}, Page number: {page['page_number']}\n"
            f"Summary: {page['text_summary']}\n"
            f"Image Analysis: {page['image_explanation']}\n"
        )

    rel_conunt = count_tokens(combined_relevant_content)
    if rel_conunt > 120000:
        combined_relevant_content = shrunk_combined_relevant_content
        combined_relevant_content = preprocess_text(combined_relevant_content)
        rel_conunt = count_tokens(combined_relevant_content)
        if rel_conunt > 127000:
            logging.error(
                f"The document contents relevant to the content are more than my context"
            )
            return
